...
// Single source of truth for the per-runtime config-home env overrides advertised
// in --help: derived from each descriptor's configHome.env so the help text can
// never drift from the descriptors (a hand-maintained list shipped the wrong pi
// env names). Order follows the registry's own runtime order; duplicates (e.g.
// XDG_CONFIG_HOME shared by kilo/opencode) are collapsed, preserving first order.
const GSD_CONFIG_ENV_VARS = ((_capabilityRegistry?.runtimes
  ? Object.values(_capabilityRegistry.runtimes).flatMap((r) => r?.runtime?.configHome?.env ?? [])
  : [])
...
function selectRuntimesFromArgs(runtimeArgs) {
  if (runtimeArgs.includes('--all')) {
    return ['claude', 'kimi', 'kilo', 'opencode', 'gemini', 'codex', 'copilot', 'antigravity', 'cursor', 'windsurf', 'augment', 'trae', 'qwen', 'hermes', 'codebuddy', 'cline', 'pi'];
  }
...
const banner = '\n' +
  ...
  '  development workflows for Claude Code, OpenCode, Gemini, Kimi CLI, Kilo, Codex, Copilot, Antigravity, Cursor, Windsurf, Augment, Trae, Qwen Code, Hermes Agent, Cline, CodeBuddy and Pi.\n';
...
function writeManifest(configDir, runtime = 'claude', options = {}) {
  ...
  if (!isCodex && !isCopilot && !isCline && !isKimi && !isPi) {
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline && !isKimi && !isPi) {
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline && !isKimi && !isPi && fs.existsSync(hooksLibSrc)) {
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  if (runtime === 'pi') command = '/gsd-new-project';
...
const allRuntimes = ['claude', 'antigravity', 'augment', 'cline', 'codebuddy', 'codex', 'copilot', 'cursor', 'gemini', 'hermes', 'kimi', 'kilo', 'opencode', 'qwen', 'trae', 'windsurf', 'pi'];
...
function buildRuntimePromptText() {
  ...
  ${cyan}17${reset}) Pi           ${dim}(~/.pi/agent)${reset}
  ${cyan}18${reset}) All
